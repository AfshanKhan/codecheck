...
client-script or an `.html` template. In a `.js` file, Frappe apps should
build UI through frm/dialog field APIs instead. In an `.html` Jinja
template, the same raw tag means a form input Frappe isn't managing at all.
The regex is markup-agnostic, so both file types are scanned the same way."""
